...
void* do_commit_abort(long t, char status) {
    ...
    zgt_tx* tx = get_tx(t);
    if (status == TR_ABORT) {
        fprintf(ZGT_Sh->logfile, "T%d\tAbortTx\t", t);	// Write log record and close
        fflush(ZGT_Sh->logfile);
    }
    else {
        fprintf(ZGT_Sh->logfile, "T%d\tCommitTx\t", t);	// Write log record and close
        fflush(ZGT_Sh->logfile);
    }
    if (tx == NULL)
    {
        fprintf(ZGT_Sh->logfile, "\n Transaction does not exsist\n");
        fflush(ZGT_Sh->logfile);
    }
    else {
        tx->status = status;
        ...
        int i, waitTxnCnt;
        if (relTx != -1) {
            waitTxnCnt = zgt_nwait(relTx);
            for (i = 1; i <= waitTxnCnt; i++)
                zgt_v(relTx);
        }
    }
}
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1) {
    ...
    /*
      Now  set the lock by transaction(T1) on an object if:
        if object has no lock then grant the lock to Transaction(T1)
        else if the object is locked by the same transaction(T1) then proceed with R/W operation
        else if the lock is hold by other transaction(T2) and
            put wait on transaction 2 
    */

    zgt_hlink* objtx = ZGT_Ht->find(sgno1, obno1);
    ...
    else if (tid1 == objtx->tid) {
        tx->status = TR_ACTIVE;
        if (lockmode1 == 'X')
            objtx->lockmode = lockmode1;
        zgt_v(0);
...
void zgt_tx::perform_readWrite(long tid, long obno, char lockmode) {
    ...
    usleep(ZGT_Sh->optime[tx->tid]);

    if (lockmode == 'S') //read operation decrement the value of obj
    {
    ...
    else {  //write operation increment the value of obj
        int obval = ZGT_Sh->objarray[obno]->value;
        ZGT_Sh->objarray[obno]->value = obval + 1;
